module ApplicationHelper
  ...
  def display_nav_item(displaytext, controllername, linkurl)
    raw("<li#{controllername == controllername ? ' class="active"' : ''}>#{link_to displaytext, linkurl}</li>")
  end
  ...
  def user_logged_in_msg
    ...
    if user_signed_in?
      str += "<p id=""user_info"">#{current_user}<br>#{link_to('Logout', destroy_user_session_path,
                                                               :method => :delete)}</p>"
    else
      str += "<p id=""user_info"">#{link_to('Login', new_user_session_path)} | #{link_to('Register',
